...
echo "####################################################################################";
echo "  ________                                   __  .__                                ";
echo " /  _____/  ____   ____   ________________ _/  |_|__| ____    ____   /\             ";
echo "/   \  ____/ __ \ /    \_/ __ \_  __ \__  \\   __\  |/    \  / ___\  \/             ";
echo "\    \_\  \  ___/|   |  \  ___/|  | \// __ \|  | |  |   |  \/ /_/  > /\             ";
echo " \______  /\___  >___|  /\___  >__|  (____  /__| |__|___|  /\___  /  \/             ";
echo "        \/     \/     \/     \/           \/             \//_____/                  ";
echo "                                                                                    ";
echo "                                                                                    ";
...
echo "                                                                                    ";
echo "                                                                                    ";
echo "   _____  __      ____________      /\    _____  ________  ________      _____      ";
echo "  /  _  \/  \    /  \______   \    / /   /  _  \ \______ \ \______ \    /     \     ";
echo " /  /_\  \   \/\/   /|       _/   / /   /  /_\  \ |    |  \ |    |  \  /  \ /  \    ";
echo "/    |    \        / |    |   \  / /   /    |    \|    .   \|    .   \/    Y    \   ";
echo "\____|__  /\__/\  /  |____|_  / / /    \____|__  /_______  /_______  /\____|__  /   ";
echo "        \/      \/          \/  \/             \/        \/        \/         \/    ";
echo "                                                                                    ";
echo "####################################################################################";
...
if [ "$oschoice" == "Y" ]
  then
	...
	_banner "COLLECTING INFORMATION SPECIFIC TO O/S" >> $destdir/osinfo.log
	echo "Copy the O/S info files:" >> $destdir/osinfo.log
	...
	_banner "COLLECTING INFORMATION SPECIFIC TO PLATFORM" >> $destdir/osinfo.log
	echo " AIX: For AIX"
	echo " HP: For HP-UX"
	echo " LX: For Linux"
	echo " SL: For Solaris"
	...
	case "$platform" in
        	...
        	#####################################################
                	_banner "Your Platform is IBM-AIX and Collecting Data " >> $destdir/osinfo.log
                	echo "==================== File system Info ================" $destdir/osinfo.log
                	df -g >> $destdir/osinfo.log
                	_printNewline >> $destdir/osinfo.log ## for new line.
                	echo "Collect the environment variables set in profle." >> $destdir/osinfo.log
                	echo ~/.profile >> $destdir/osinfo.log
                	_printNewline >> $destdir/osinfo.log ## for new line.
                	echo "==== Swap Info and Memory =====" >> $destdir/osinfo.log
                	/usr/sbin/lsattr -E -l sys0 -a realmem  >> $destdir/osinfo.log
                	_printNewline >> $destdir/osinfo.log ## for new line.
                	/usr/sbin/lsps -a >> $destdir/osinfo.log
        	;;
    		'HP')
       		#####################################################
        	# Your Platform is HP-UX and Collecting Data        #
        	#####################################################
                	_banner "Your Platform is HP-UX and Collecting Data" >> $destdir/osinfo.log
                	echo "==================== File system Info ================" $destdir/osinfo.log
                	bdf >> $destdir/osinfo.log
                	_printNewline "\n" >> $destdir/osinfo.log ## for new line.
                	echo "Collect the environment variables set in profle." >> $destdir/osinfo.log
                	echo ~/.profile >> $destdir/osinfo.log
                	_printNewline >> $destdir/osinfo.log ## for new line.
                	echo "==== Swap Info and Memory =====" >> $destdir/osinfo.log
                	/usr/contrib/bin/machinfo | grep -i Memory >> $destdir/osinfo.log
                	_printNewline >> $destdir/osinfo.log ## for new line.
               		/usr/sbin/swapinfo -a >> $destdir/osinfo.log
        	;;
    		'LX')
        	#####################################################
        	# Your Platform is linux and Collecting Data        #
        	#####################################################
                	_banner "Your Platform is Linux and Collecting Data" >> $destdir/osinfo.log
                	echo "==================== File system Info ================" >> $destdir/osinfo.log
                	df -h >> $destdir/osinfo.log

               		_printNewline >> $destdir/osinfo.log ## for new line.
                	echo "Collect the environment variables set in profle." >> $destdir/osinfo.log
                	echo ~/.bash_profile >> $destdir/osinfo.log

                	_printNewline >> $destdir/osinfo.log ## for new line.

                	echo "==== Swap Info and Memory =====" >> $destdir/osinfo.log
                	cat /proc/meminfo | grep "Mem" >> $destdir/osinfo.log

                	_printNewline >> $HOME/DBHC/osinfo.log ## for new line.
                	cat /proc/meminfo | grep "Swap" >> $destdir/osinfo.log
        	;;
    		'SL')
        	#####################################################
        	# Your Platform is Solaris and Collecting Data      #
       		#####################################################
        		 _banner "Your Platform is Solaris and Collecting Data"  >> $destdir/osinfo.log
			echo "==================== File system Info ================" >> $destdir/osinfo.log
			df -h >> $destdir/osinfo.log
			_printNewline >> $destdir/osinfo.log ## for new line.
			echo "Collect the environment variables set in profle." >> $destdir/osinfo.log
			...
			_printNewline >> $destdir/osinfo.log ## for new line.
			echo "==== Swap Info and Memory =====" >> $destdir/osinfo.log
			/usr/sbin/prtconf | grep "Memory size" >> $destdir/osinfo.log
			_printNewline >> $destdir/osinfo.log ## for new line.
			/usr/sbin/swap -s >>$destdir/osinfo.log
		;;
	esac
else 
echo -e "\n"
