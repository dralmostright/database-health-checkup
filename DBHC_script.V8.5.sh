...
where sysdate - START_TIME < 30 order by START_TIME desc; 
...
order by to_date("Date", 'DD-MON-RR') desc;
EOF
...
where   end_interval_time >= (sysdate-4/24)
...
define begin_time="${ASH_BGNTIME}";
define duration=${ASH_INTERVAL};
define report_type=${ASH_RPT_FORMAT};
...
elif [ "$DBMODE" = "READ ONLY WITH APPLY" ] && [ "$DBROLE" = "PHYSICAL STANDBY" ];
	then
	spoolDBFULL $ORACLE_HOME $FILE_NAME
else 
	echo "Database is in Read Only with Apply Mode."
fi
...
ADDM_TNAME="DBHC_$DATE_TIME"
ADDM_TDESC="ADDM for DBHC $DATE_TIME"
ASH_RPT_FILE=$destdir"/Ash_report_"$ORA_INSTANCE"_DBHC_"$DATE_TIME".html"
...
if [ "$DBMODE" = "READ WRITE" ] && [ "$DBROLE" = "PRIMARY" ];
	...
	spoolASHRpt $ORACLE_HOME $ASH_RPT_FILE
else
